...
def verify_fact(question, correct_answer):
    ...
    FACT_CHECK_CACHE[cache_key] = random.random() > 0.2
    return FACT_CHECK_CACHE[cache_key]
        ...
        if избор in налични_помощници:
            ...
            if използван == "50:50":
                ...
                for к in за_премахване:
                    del отговори[к]
                говори("Избрахте жокер 50 на 50. Оставащите отговори са: ")
...
def display_question(въпрос, текуща_награда, отговори, speak=True):
    ...
    question_lines = [въпрос['въпрос'][i:i+80] for i in range(0, len(въпрос['въпрос']), 80)]
    ...
    for key in ['A', 'B', 'C', 'D']:
        if key in отговори:
            answer = f"{key}) {отговори[key]}"
            print_centered(answer)
...
def стартирай_игра():
    ...
    for въпрос in предварителни_въпроси():
        ...
        if избор == въпрос['верен']:
            ...
            говори(f"Съжалявам, верният отговор беше {bg_letter}) {въпрос['отговори'][верен_отгород]}")
